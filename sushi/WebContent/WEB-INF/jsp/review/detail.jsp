...
<table>
	...
	<tr>
		<td class="cont" colspan="2">${review.content}</td>
	</tr>
	
	<tr>
		<td colspan="2"> <c:if test="${member.id eq review.id }"><a href="<%=request.getContextPath()%>/review/list"> <button id="delbtn">삭제</button></a> </c:if><c:if test="${member.id eq review.id}"><a href="reviewUpdate?bno=${review.bno}"><button>수정</button></a></c:if><a href="<%=request.getContextPath()%>/review/list"><button>목록</button></a></td>
	</tr>
...
$("#delbtn").click(function(){
	...
	var bno = ${review.bno}
	var url = cp + "/review/delete?bno="+bno ;
